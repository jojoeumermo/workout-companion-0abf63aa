...
import { Play, Dumbbell, TrendingUp, Flame } from 'lucide-react';
import { motion } from 'framer-motion';
import PageShell from '@/components/PageShell';
import { useHistory, useTemplates } from '@/hooks/useStorage';
...
export default function Dashboard() {
  ...
  const stats = [
    { icon: Flame, label: 'Treinos', value: weekCount, suffix: 'esta semana' },
    { icon: TrendingUp, label: 'Volume', value: weekVolume > 1000 ? `${(weekVolume / 1000).toFixed(1)}t` : `${weekVolume}kg`, suffix: 'esta semana' },
  ];
    ...
    <PageShell>
      <div className="pt-14 space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-2 gap-4">
          {stats.map(s => (
            <div key={s.label} className="bg-card rounded-2xl p-4 space-y-2">
              <s.icon size={20} className="text-primary" />
              <p className="text-2xl font-bold">{s.value}</p>
              <p className="text-xs text-muted-foreground font-body">{s.suffix}</p>
            </div>
          ))}
        </motion.div>
